...
#
# This is a modified version of calculate_crystal_fluctuations.py
# which has been optimized for small-memory computers. It uses less
# memory at the cost of running more slowly.
...
# The name of the PDB file to be read. Note that this file must contain
# the crystal parameters as well as the atomic coordinates.
...
# Optimized diagonalization for complex hermitian matrices.
# This works much like LA.Heigenvectors, but overwrites its
# input array.
if N.package == 'NumPy':
...
def diagonalizeComplex(a):
    n = a.shape[0]
    liwork = 5*n+3
    iwork = N.zeros((liwork,), N.Int32)
    w = N.zeros((n,), N.Float)
    lwork = 1
    work = N.zeros((lwork,), N.Complex)
    lrwork = 1
    rwork = N.zeros((lrwork,), N.Float)
    results = lapack_lite.zheevd('V', 'L', n, a, n, w, work, -1,
                                 rwork, -1, iwork, liwork,  0)
    lwork = int(abs(work[0]))
    work = N.zeros((lwork,), N.Complex)
    lrwork = int(rwork[0])
    rwork = N.zeros((lrwork,), N.Float)
    results = lapack_lite.zheevd('V', 'L', n, a, n, w, work, lwork,
